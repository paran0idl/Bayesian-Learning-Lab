...
pdraw=rmvnorm(1000,mean=posterior_mode,sigma = posterior_cov)
NSmallChild=pdraw[,7]
NSmallChild=sort(NSmallChild)
lower_value=NSmallChild[length(NSmallChild)*0.025]
upper_value=NSmallChild[length(NSmallChild)*0.975]
ggplot()+geom_density(aes(NSmallChild))+geom_vline(xintercept = lower_value)+geom_vline(xintercept = upper_value)
```

The plot above shows the density and 95% credible interval of varaible NSmallChild from 1000 simulations. As we can see from it that 95% of simulation were gather between -0.6 to -2.2, based on this we can say this variable has a negetive impact on the result. 
