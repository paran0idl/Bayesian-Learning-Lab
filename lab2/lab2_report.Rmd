...
Based on our intuition, the highest tempreture in a year should be around July or August. We take the first derivative of time and then plot the distribution, the highest result is around 0.542 which indicate the result meet our intuition.
...
For example, we can choose laplace prior $\beta_i|\sigma^2~Laplace(0,\frac{\sigma^2}{\lambda})$. For laplace prior, many $\beta_i$ close to zero but some of the $\beta_i$ could have really large values, which meet our needs.
...
From the plot, we can observed that 95% credible interval does not contain 0, this feature is an important determinant. Using R built-in model glm to anaylize the given data, it is shown that very small p-value in frequentist approach, indicating it is very much unlikely that NSmallChild has no effect on the response variable.
...
res=c()
for(i in 1:10000){
  posterior_draws=rmvnorm(10,mean=posterior_mode,sigma = posterior_cov)
  new_woman_res=posterior_draws%*%new_woman
  working_women=length(new_woman_res[new_woman_res>0])
  res=c(res,working_women)
}
plot(density(res))
```
